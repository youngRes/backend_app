...
Window_TitleCommand.prototype.makeCommandList = function() {
    ...
    if ($userName != null) {
        this.addCommand("Usuario: " + $userName,   'loginUser');
        this.addCommand(TextManager.loadChapter,   'loadChapter');
    }else{
        this.addCommand(TextManager.loginUser,   'loginUser');
    }
...
// Window_MapList
//
// The window for selecting a chapter on the chapter File Selected.
...
Window_MapList.prototype.initialize = function(x, y, width, height) {
    Window_Selectable.prototype.initialize.call(this, x, y, width, height);
    this.activate();
    this._mode = null;
};

Window_MapList.prototype.setMode = function(mode) {
    this._mode = mode;
};

Window_MapList.prototype.maxItems = function() {
    return $dataChapterInfos.length-1;
};

Window_MapList.prototype.maxVisibleItems = function() {
    return 5;
};
...
Window_MapList.prototype.drawItem = function(index) {
    var id = index + 1;
    var valid = true;
    var info = DataManager.loadChapterInfo(id);
    var rect = this.itemRectForText(index);
    this.resetTextColor();
    if (this._mode === 'load') {
        this.changePaintOpacity(valid);
...
Window_MapList.prototype.drawFileId = function(id, x, y) {
    this.drawText(TextManager.file + ' ' + id, x, y, 180);
};

Window_MapList.prototype.drawContents = function(info, rect, valid) {
    var bottom = rect.y + rect.height;
    if (rect.width >= 420) {
        this.drawGameTitle(info, rect.x + 192, rect.y, rect.width - 192);
    }
...
Window_MapList.prototype.drawGameTitle = function(info, x, y, width) {
    if (info.name) {
        this.drawText(info.name, x, y, width);
    }
};

Window_MapList.prototype.playOkSound = function() {
};
